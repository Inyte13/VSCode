...
// Con objetos
let persona2 = personas2.find((persona) => {
  return persona.name === "Luna";
}); // {name: 'Luna'}

// findIndex, recibe una función callback que busca al elemento y si lo encuentra devuelve el índice, sino -1
// Con un arr
let i3 = personas.findIndex((persona) => {
  return persona === "cam";
}); // -1
// Con objetos
let i4 = personas2.findIndex((persona) => {
  return persona.name === "Lydia";
}); // 3

// some, recibe una función callback retorna true si alguna es true
let torf2 = nums.some((x) => {
  return x % 2 === 0;
}); // true
